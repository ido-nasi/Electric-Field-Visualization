...
class Spot:
    ...
    def update_field(self, charges):
        ...
        for i in range(0, len(self.arrow_points)):
            new.append(rotate_point((self.x + self.width // 2, self.y + self.width // 2), self.arrow_points[i], angle))
...
def update_field(grid, charges):
    for row in grid:
        for spot in row:
            spot.update_field(charges)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    update_field(grid, current_charges)
